...
const Hero = () => {
  ...
  return (
    <div className="w-full">
      ...
      >
        <Link to={`https://${user.instagramURL}`} target="_blank">
          <Instagram className="text-pink-500 w-7 h-7" />
        </Link>
        <Link to={`https://${user.facebookURL}`} target="_blank">
          <Facebook className="text-blue-800 w-7 h-7" />
        </Link>
        <Link to={`https://${user.linkedInURL}`} target="_blank">
          <Linkedin className="text-sky-500 w-7 h-7" />
        </Link>
        <Link to={`https://${user.twitterURL}`} target="_blank">
          <Twitter className="text-blue-800 w-7 h-7" />
        </Link>
      </div>
      <div className="mt-4 md:mt-8 lg:mt-10  flex gap-3">
        <Link to={`https://${user.githubURL}`} target="_blank">
